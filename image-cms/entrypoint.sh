...
if [ "$1" = 'cms' ]; then
    ...
    git clone https://github.com/ooemperor/PSEGrader.git
    python3 -m ensurepip
    ...
    # Replace all instances of time.clock() with time.time()
    find /path/to/cms -type f -name "*.py" -exec sed -i 's/time.clock()/time.time()/g' {} +
